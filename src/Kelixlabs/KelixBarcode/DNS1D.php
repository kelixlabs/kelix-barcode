...
class DNS1D
{
    ...
    protected function barcodeI25($code, $checksum = false)
    {
        ...
        for ($i = 0; $i < $clen; $i = ($i + 2)) {
            $char_bar = $code{$i};
            $charSpace = $code{$i + 1};
            if ((!isset($chr[$char_bar])) or (!isset($chr[$charSpace]))) {
                // invalid character
                return false;
            }
            ...
            for ($s = 0; $s < $chrlen; $s++) {
                $seq .= $chr[$char_bar]{$s} . $chr[$charSpace]{$s};
            }
            $seqlen = strlen($seq);
            for ($j = 0; $j < $seqlen; ++$j) {
                if (($j % 2) == 0) {
     ...
     * @param $code (string) code to print, separate the ZIP (routing code) from the rest using a minus char '-' (BarcodeIDServiceTypeIDMailerIDSerialNumber-RoutingCode)
     * @return array barcode representation.
     * @protected
     */
    protected function barcode_imb($code) {
        ...
        $binaryCode = bcadd($binaryCode, $tracking_number{0});
        $binaryCode = bcmul($binaryCode, 5);
        $binaryCode = bcadd($binaryCode, $tracking_number{1});
        $binaryCode .= substr($tracking_number, 2, 18);
        // convert to hexadecimal
        $binaryCode = $this->dec_to_hex($binaryCode);
        // pad to get 13 bytes
        $binaryCode = str_pad($binaryCode, 26, '0', STR_PAD_LEFT);
        // convert string to array of bytes
        $binaryCode_arr = chunkSplit($binaryCode, 2, "\r");
        $binaryCode_arr = substr($binaryCode_arr, 0, -1);
        $binaryCode_arr = explode("\r", $binaryCode_arr);
        // calculate frame check sequence
        $fcs = $this->imbCrc11fcs($binaryCode_arr);
        // exclude first 2 bits from first byte
        $first_byte = sprintf('%2s', dechex((hexdec($binaryCode_arr[0]) << 2) >> 2));
        $binaryCode_102bit = $first_byte . substr($binaryCode, 2);
        // convert binary data to codewords
        $codewords = array();
        $data = $this->hex_to_dec($binaryCode_102bit);
        ...
        $bararray = array('code' => $code, 'maxw' => 0, 'maxh' => 3, 'bcode' => array());
        for ($i = 0; $i < 65; ++$i) {
            $asc = (($characters[$ascChr[$i]] & pow(2, $asc_pos[$i])) > 0);
            $dsc = (($characters[$dscChr[$i]] & pow(2, $dsc_pos[$i])) > 0);
            if ($asc and $dsc) {
                // full bar (F)
                $p = 0;
                $h = 3;
    ...
    protected function imb_tables($n, $size)
    {
        ...
        for ($count = 0; $count < 8192; ++$count) {
            ...
            for ($bit_index = 0; $bit_index < 13; ++$bit_index) {
                $bitCount += intval(($count & (1 << $bit_index)) != 0);
            }
